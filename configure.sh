...
if [ $STEP -eq 0 ]; then
    ...
    for h in $HOSTS; do
        ...
        if [ $h != $(hostname -s) ]; then
          scp -r $SSHOPTS /etc/puppet/data $USER@$h.$DOMAIN:/etc/puppet/
        fi
    done
    puppet apply /tmp/manifest.pp | tee  $LOGDIR/puppet-master.step0.log
    puppet apply /tmp/manifest2.pp | tee  $LOGDIR/puppet-master.step0.log
    sleep 30
